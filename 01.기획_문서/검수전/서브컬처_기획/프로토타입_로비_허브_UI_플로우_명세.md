...
- **서브컬처 감상 허브**: 캐릭터의 동조율 단계를 파악하고, 해금된 개인 심상 기록을 감상하며, 결과에 따른 대사 변화를 통해 캐릭터와의 정서적 연대를 확인합니다. 이때 기억 파편은 창고에 전혀 보관되지 않으며, 탈출 결과 화면에서 즉시 전량 소모되어 동조율(호감도)을 상승시키고 레벨업 시 심상 기록을 해금하는 정산 자원으로 처리됩니다.
...
- **기억 파편 제외**: 기억 파편은 창고/인벤토리에 보관되거나 화면에 노출되지 않으며, 결과 화면에서 즉시 소모/처리되므로 창고 보관 리스트에서 완전히 제외됩니다.
...
- **기억 파편의 창고 노출 제외**:
  - 기억 파편은 결과 화면에서 직접 소모/정산되는 휘발성 정산 자원이므로 창고 및 인벤토리 슬롯 영역에서는 완전 제외됩니다. (※ UI 시안 상의 하단 팁 영역 등은 단순 가이드이며, 실제 보관 및 관리는 창고 화면을 거치지 않습니다.)
...
| 상태 구분 | 세션 요건 | 기억 파편 처리 | 일반 회수품 처리 | 동조율(호감도) 변화 | 심상 기록 해금 여부 | 로비 알림(NEW) |
| :--- | :--- | :--- | :--- | :---: | :---: | :---: |
| **경우 1. 성공적 귀환** | 탈출 성공 및 파편 1개 이상 | 결과 화면 즉시 소모 (획득수만큼) | 마나석/회복약 창고 누적 저장 | 획득수만큼 동조율 상승 (레벨업 가능) | 레벨업 시 신규 기록 해금 (`true`) | 다이버 메뉴 활성화 |
| **경우 2. 일반 귀환** | 탈출 성공 및 파편 0개 | 소모 없음 (0개) | 마나석/회복약 창고 누적 저장 | 변동 없음 | 해금 없음 (`false`) | 비활성화 |
| **경우 3. 강제 각성** | 세션 내 패배 / 사망 | 파편 전체 유실 (소모 X) | 획득 회수품 전체 유실 | 변동 없음 (동조율 상승 X) | 해금 없음 (`false`) | 비활성화 |
...
- **기억 파편 처리**: 창고에 보관하지 않으며, 탈출 성공 시 결과 화면에서 즉시 전량 소모하여 동조율(호감도) 경험치를 올리고 레벨업 시 심상 기록 해금
...
- **기억 파편**은 창고에 보관하지 않으며, 탈출 성공 시 결과 화면에서 즉시 소모(사망 시 소모 없이 유실)되어 동조율 상승 및 기록 해금으로 이어지는 정산용 자원입니다.
