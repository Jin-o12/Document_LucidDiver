...
- **크리에이티브 원안의 시스템 명세화**:
  - 장수영 CP의 정의 및 원안을 바탕으로 개발 구현이 가능한 마스터 명세서를 수립 및 구체적 명세화:
    ...
    - [P0.5_꿈식자_에너미_AI_및_컨셉_기획서_v1.0.0.md](../../../LucidDiver_Document/03_기획_부록_참고문서/컨셉_기획서/P0.5_꿈식자_에너미_AI_및_컨셉_기획서_v1.0.0.md)
    ...
    - [베타빌드_통합_정의서.md](../../../LucidDiver_Document/01_SSOT_최종_기준문서/베타빌드_통합_정의서.md)
...
- **AudioManager 및 IAudioRepository 아키텍처**:
  - 장수영 CP가 제공한 58종 SFX 및 BGM 리소스를 관리하는 [AudioManager.cs](../../../../UnityProject_LucidDiver/Assets/02_Scripts/Manager/Manager/AudioManager.cs) 및 `IAudioRepository` 구현.
...
- **다크 스피릿 FSM 알고리즘**: `EnemyBrain`, `EnemyLocomotion`, `EnemyAttack` 기반의 Idle ➡️ Alert ➡️ Chase ➡️ Attack ➡️ Dead 상태 추격 및 소음/시야 반응 제어.
- **시야 감지 및 그림자 동기화 버그 리팩토링**:
  - 시야 재진입 시 몬스터 포트레이트 본체와 그림자(Shadow) 잔상이 지면에 겹쳐 박제되던 결함을 분석.
...
- **결과 창 정산 및 탈출 처리 체계**:
  ...
  - 정산 시 Addressables 기반 아이콘 동적 로드 및 동조율(`LinkRateUp`) 상승 연산, 실시간 킬 카운터 구현.
- **로비 대사 동조율 레벨 필터링**:
...
| **오디오** | `assets/audio/BGM_Mart_Theme.mp3` | FMOD Dual Fallback 체계 및 BGM 루프 연동 오디오 |
...
public void PlaySFX3D(string sfxName, Vector3 position)
{
    // AudioMixer 3D 공간 거리 감쇄 연산 및 풀링
    AudioMixer.SetFloat("AmbVolume", currentAmbVolume);
}
```
