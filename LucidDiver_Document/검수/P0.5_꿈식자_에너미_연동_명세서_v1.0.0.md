...
| `LMK-006` | 낙인 적용 시 화면 외곽을 어둡게 하는 비네팅(Vignette) 효과가 단계별로 차등 인가(1단계: 0.35, 2단계: 0.70, 3단계: 0.95)되어, 손전등 범위 외의 주변 시인성을 크게 제한한다. |
...
| 낙인 | `lucidMarkVignetteIntensityStage3` | `float` | 3단계 외곽 비네팅 시야 차단 강도 (기본값: 0.95) |
...
| `QA-010` | 루시드 낙인 1단계, 2단계 및 3단계 중첩 시 화면 외곽 비네팅(어둠 왜곡) 강도가 설정 수치(0.35, 0.70, 0.95)에 맞게 실시간 연동되어 스크린 시인성이 감소하고, 3단계 진입 시 이명 사운드가 지속 출력되는지 확인한다. |
