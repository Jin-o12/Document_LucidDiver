...
- **목적**: `꿈식자 에너미 AI 및 컨셉 기획서 (v1.0.1)`를 현재 프로젝트의 기본 에너미 구현에 실제로 연결하기 위해 필요한 기술 명세를 정의한다. 본 명세는 기획적 요구 사항을 개발자가 바로 작업 가능한 수준의 구현 단위로 구조화하는 데 초점을 맞춘다.
- **핵심 가치**: 기존의 단일 추적형 적 구조(`EnemyStatus`, `EnemyMovement`)를 모듈형 상태 머신 구조로 확장하여, `소리 감지`, `조사`, `먹잇감 각인`, `추격 기억`, `먹잇감 차단 이동`, `루시드 낙인`, `괴성 전파`, `전진 2연격`, `광폭화` 시스템을 유기적으로 연동한다.
...
| **행동 제어 (FSM)** | - `idle`, `detect`, `attack`, `hit`, `dead` (5개 상태) | - `Patrol`, `Suspicious`, `Investigate`, `AlertCry`, `Chase`, `Intercept`, `AttackReady/Slash1/Slash2/Recovery`, `ChaseMemory`, `Search`, `Berserk`, `Dead` (15개 상태) | - 소리 조사, 각인 추격, 미래 예측 차단 이동, 전조 2연격 및 광폭화 등 고도화된 전술 행동 묘사 |
...
| **추격 품질 및 보정** | - 플레이어의 현재 좌표를 단순 추적 | - 플레이어의 진행 방향 및 속도를 분석한 짧은 미래 예측형 차단 이동 (`Intercept`) | - 적이 허공을 쫓아 맴도는 문제 개선 및 추격의 압박감 향상 |
| **피격 압박 요소** | - 단순 대미지 피해 후 해제 | - 피격 시 플레이어에게 `루시드 낙인` 부여 (기억 시간 연장, 2단계 시 잔향으로 주변 적 추가 유도) | - 대미지 수치를 과도하게 올리지 않고도 피격에 대한 위험 감각 및 조심성 부여 |
| **무리 반응 규칙** | - 단독 또는 개별 반응 (주변 적 무반응) | - 개체의 괴성(`AlertCry`), 소음, 또는 루시드 잔향을 주변 적들이 청각 감지해 해당 지점으로 수사 및 합류 | - 군인처럼 전원의 실시간 좌표를 공유하지 않고, 굶주린 짐승처럼 울음/소리로 조사 접근하게 유도 |
...
- **예측 알고리즘 (`INT-001`)**: 플레이어의 속도 벡터와 최근 방향 데이터를 기반으로 `interceptPredictionTime` 미래의 위치를 계산하고, `maxInterceptOffsetDistance` 한계 내에서 타겟 이동 지점을 보정하여 앞서 막아서는 경로로 `NavMeshAgent`를 설정한다.
- **적용 제약 (`INT-002`)**: 직접 시야가 유효하고 플레이어 이동 속도가 `interceptMinTargetSpeed` 이상일 때에만 `Chase` 도중 활성화하며, 시야 상실 시나 정지 시에는 일반 플레이어 좌표 추적으로 즉각 복귀한다.

#### 2.2.4 루시드 낙인 규칙 (Lucid Mark Rules)
- **낙인 1단계 (`MRK-001`)**: 할퀴기 피격 시 대상 플레이어에게 `lucidMarkDuration`초간 1단계 낙인을 부여한다. 낙인 대상은 추격 기억 유예 시간에 `lucidMarkMemoryBonusTime` 보너스가 가산되고, 최대 추격 거리 한계도 `lucidMarkLeashBonusDistance`만큼 늘어나 해제가 더 어려워진다.
- **낙인 2단계 및 잔향 (`MRK-002`)**: 낙인이 활성화된 동안 추가 피격 시 2단계로 갱신되며, 피격 원점 바닥에 `lucidResidueDuration`초간 유지되는 `루시드 잔향` 오브젝트를 생성한다.
- **잔향의 전파 (`MRK-003`)**: 잔향 스폰 시 주변 `lucidResidueInvestigateRange` 이내의 다른 꿈식자 개체들에게 청각 이상 감지 이벤트를 발송하여, 잔향 발생 원점으로 강제 조사를 유도한다.
- **조작성 유지 제약 (`MRK-004`)**: 루시드 낙인은 플레이어의 강제 조작권 박탈(슬로우, 경직 연장, 상호작용 강제 캔슬 등)을 일으키지 않는다.
...
| **스탯** | 최대 체력 | `maxHp` | `float` | 에너미 최대 체력 (기본값: 100.0) |
| **스탯** | 기본 공격 피해 | `baseAttackDamage` | `float` | 기본 할퀴기 공격 피해 |
| **이동** | 일반 이동 속도 | `normalMoveSpeed` | `float` | 일반 상태에서의 순찰 및 추격 속도 (기본값: 3.2) |
| **이동** | 광폭화 이동 배율 | `berserkMoveSpeedMultiplier` | `float` | 광폭화 시 이동 배율 (기본값: 1.18) |
| **인지** | 직접 시야 거리 | `sightRange` | `float` | 직접 시야 감지 범위 (기본값: 5.5) |
| **인지** | 직접 시야 각도 | `sightAngle` | `float` | 직접 시야 감지 각도 (정면 기준) (기본값: 100.0) |
| **인지** | Raycast 눈높이 | `eyeHeight` | `float` | 벽 가림 검사용 Raycast 시작 오프셋 (기본값: 1.6) |
| **인지** | 근접 인지 반경 | `proximityAwarenessRange` | `float` | 후방/측면 기척 감지 보정 반경 (기본값: 1.8) |
...
| **소리** | 최소 반응 지연 | `soundReactionDelayMin` | `float` | 소리 감지 후 이동 시작까지의 최소 딜레이 (기본값: 0.4) |
| **소리** | 최대 반응 지연 | `soundReactionDelayMax` | `float` | 소리 감지 후 이동 시작까지의 최대 딜레이 (기본값: 1.2) |
| **추격** | 시야 상실 유지 거리 | `chaseMemoryRange` | `float` | 시야 상실 후 기억을 유지할 거리 (기본값: 11.0) |
| **추격** | 최대 추격 거리 | `leashRange` | `float` | 어그로 해제 및 복귀 한계 거리 (기본값: 17.0) |
| **추격** | 시야 상실 유예 시간 | `lostSightGraceTime` | `float` | 시야 상실 후 어그로 유지 버퍼 시간 (기본값: 2.5) |
| **추격** | 마지막 위치 탐색 시간 | `searchDuration` | `float` | 마지막 발견 위치 탐색 유지 시간 (기본값: 4.0) |
| **추격** | 소리 기억 추가 시간 | `noiseMemoryAddTime` | `float` | 탐색 중 추가 소리 감지 시 어그로 연장 시간 (기본값: 1.5) |
| **차단** | 예측 시간 | `interceptPredictionTime` | `float` | 짧은 미래 위치 예측 시간 (기본값: 0.35) |
| **차단** | 최대 차단 오프셋 거리 | `maxInterceptOffsetDistance` | `float` | 예측 위치의 최대 전방 보정 거리 (기본값: 1.8) |
| **차단** | 적용 최소 속도 | `interceptMinTargetSpeed` | `float` | 플레이어가 이 속도 이상일 때만 적용 (기본값: 1.5) |
| **차단** | 차단 갱신 주기 | `interceptUpdateInterval` | `float` | 예측 위치 갱신 주기 (기본값: 0.2) |
| **차단** | 시야 중에만 사용 | `useInterceptOnlyWhenVisible` | `bool` | 직접 시야 유지 중에만 사용 (기본값: true) |
| **낙인** | 1단계 낙인 지속 시간 | `lucidMarkDuration` | `float` | 첫 피격 후 유지 시간 (기본값: 6.0) |
| **낙인** | 2단계 낙인 지속 시간 | `lucidMarkStage2Duration` | `float` | 강화 시 유지 시간 (기본값: 8.0) |
| **낙인** | 낙인 추격 기억 추가 시간 | `lucidMarkMemoryBonusTime` | `float` | 낙인 대상 추격 유지 강화 시간 (기본값: 2.0) |
| **낙인** | 낙인 최대 추격 거리 보너스 | `lucidMarkLeashBonusDistance` | `float` | 낙인 대상 이탈 허용 거리 증가분 (기본값: 2.0) |
| **낙인** | 2단계 잔향 지속 시간 | `lucidResidueDuration` | `float` | 잔향이 남아 있는 시간 (기본값: 3.0) |
| **낙인** | 잔향 조사 반경 | `lucidResidueInvestigateRange` | `float` | 주변 꿈식자의 조사 유도 반경 (기본값: 8.0) |
| **공격** | 공격 시작 거리 | `attackStartRange` | `float` | 전진 2연격 준비 진입 거리 (기본값: 2.3) |
| **공격** | 개체별 쿨타임 | `attackCooldown` | `float` | 개체별 전진 2연격 내부 쿨타임 (기본값: 1.2) |
| **공격** | 1타 전조 시간 | `firstSlashTelegraphTime` | `float` | 1타 바닥 전조 활성화 시간 (기본값: 0.35) |
| **공격** | 2타 전조 시간 | `secondSlashTelegraphTime` | `float` | 2타 바닥 전조 활성화 시간 (기본값: 0.25) |
| **공격** | 1타 전진 거리 | `firstSlashLungeDistance` | `float` | 1타 전진 길이 (기본값: 1.0) |
| **공격** | 2타 전진 거리 | `secondSlashLungeDistance` | `float` | 2타 전진 길이 (기본값: 0.8) |
| **공격** | 1타 판정 폭 | `firstSlashWidth` | `float` | 1타 공격 판정의 가로 폭 (기본값: 1.2) |
| **공격** | 2타 판정 폭 | `secondSlashWidth` | `float` | 2타 공격 판정의 가로 폭 (기본값: 1.2) |
| **공격** | 1타 피해량 | `firstSlashDamage` | `float` | 1타 공격 데미지 (기본값: 5.0) |
| **공격** | 2타 피해량 | `secondSlashDamage` | `float` | 2타 공격 데미지 (기본값: 6.0) |
| **공격** | 연격 간격 | `comboInterval` | `float` | 1타 후 2타가 발동하기까지의 딜레이 (기본값: 0.15) |
| **공격** | 공격 후딜레이 | `attackRecoveryTime` | `float` | 2연격 완료 후 후딜레이 시간 (기본값: 0.65) |
| **공격** | 2타 방향 보정 한계 | `secondSlashTurnLimit` | `float` | 2타 전 회전각 한계 (최대 25도) |
| **광폭** | 발동 HP 비율 | `berserkHpThreshold` | `float` | 광폭화가 적용되는 체력 비율 (0.5) |
| **광폭** | 전조 시간 배율 | `berserkTelegraphMultiplier` | `float` | 광폭화 시 전조 딜레이 단축 배율 (기본값: 0.8) |
| **광폭** | 전진 거리 배율 | `berserkLungeMultiplier` | `float` | 광폭화 시 전진 거리 증가 배율 (기본값: 1.15) |
| **광폭** | 후딜 배율 | `berserkRecoveryMultiplier` | `float` | 광폭화 시 후딜레이 단축 배율 (기본값: 0.85) |
...
public enum NoiseType
{
    ...
    HitReaction,
    CombatImpact,
    LucidResidue // [신규] 루시드 잔향 감지용 소음 타입
}
```
...
|---|---|---|---|
| `OnNoiseEmitted` | `PlayerMovement`, `PlayerWeapon`, `EnemyBrain`, `LucidResidue` | `EnemyNoiseListener` | `Vector3 position, NoiseType type, float range, GameObject source` |
...
| `OnPlayerLucidMarkChanged` | `EnemyCombat`, `PlayerStatus` | UI, VFX | `int playerId, int stage` |
...
| `EnemyCombat` | 1타/2타 전조 데칼 생성, 전진 공격 실행, 피격 판정 수행, 루시드 낙인 인가 | 신규 생성 |
| `EnemyPresentation` | 상태 전이에 맞춰 애니메이터 파라미터 제어 및 VFX/SFX 호출 | 신규 생성 |
| `EnemyLocomotion` | `NavMeshAgent`를 이용한 정지, 회전, 순찰 이동 처리, Intercept 보정 적용 | 기존 `EnemyMovement` 축소 버전 |
...
| `QA-011` | 공격에 피격되었을 때 플레이어에게 루시드 낙인이 알맞은 지속시간 동안 부여되며, 2단계 누적 시 피격 바닥에 루시드 잔향이 정상 스폰되는지 확인한다. |
| `QA-012` | 루시드 잔향이 생성되었을 때, 주변의 다른 대기 상태 적들이 해당 잔향 위치로 어그로 수사를 시작하는지 확인한다. |
| `QA-013` | 플레이어가 달리기 등 움직임을 취할 때, 적이 단순 타겟 추적이 아닌 타겟 진행 방향을 차단해 들어오는 예측 이동을 수행하는지 확인한다. |
...
| 2026-07-01 | v1.0.1 | - 먹잇감 차단 이동(Intercept) 및 루시드 낙인/잔향 연동 설계 추가 반영 | AI(Antigravity) |
