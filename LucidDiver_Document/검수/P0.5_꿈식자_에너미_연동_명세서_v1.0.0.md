...
flowchart TD
    ...
    AttackSlash2 --> OnHitMark2[낙인 중 추가 피격 시 Stage 2 (주변 적 즉시 추격)]
...
| `LMK-004` | 2단계 강화 시 공간에 물리적으로 표출되지 않는 내부 이벤트로 작동하며, 반경 내에 있는 미인지 꿈식자들이 플레이어를 즉시 타겟팅하고 추격을 시작한다. |
...
| 상흔 | `lucidScarSensoryRange` | `float` | 주변 꿈식자 즉시 추격 유도 반경 (기본값: 8.0) |
...
| `OnLucidScarTriggered` | `PlayerLucidMarkReceiver` 또는 `EnemyCombat` | `EnemyBrain` (주변 적들) | `Vector3 playerPosition, float range` |
...
| `PlayerLucidMarkReceiver` | 플레이어 낙인 단계 및 지속 시간 관리, 2단계 진입 시 상흔 이벤트 발생 처리 | 신규 생성 |
...
- **상흔의 역할 제한**: 루시드 상흔은 시각/청각적 이펙트가 없는 순수 내부 시스템적 연동 사양입니다. 2단계 낙인 발동 시 주변 미인지 적들이 즉시 추격 상태로 전환되는 고유의 어그로 확장 규칙으로 작동합니다.
...
| `QA-005` | 낙인 지속 중 추가 피격 시 2단계로 강화되고, 주변 적에게 즉시 추격을 유도하는 상흔 이벤트가 발동한다. |
| `QA-006` | 상흔 이벤트는 밖으로 표출되는 시/청각 효과 없이, 설정된 반경 내의 미인지 적들이 플레이어를 즉시 추격하도록 한다. |
