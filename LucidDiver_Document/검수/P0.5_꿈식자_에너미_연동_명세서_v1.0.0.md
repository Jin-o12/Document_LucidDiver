...
flowchart TD
    ...
    AttackSlash2 --> OnHitMark2[낙인 중 추가 피격 시 Stage 2 (의식 누출 위치 전달)]
...
| `LMK-002` | 낙인 지속 중 추가 피격을 받으면 `루시드 낙인 2단계`로 강화되며, 2단계 지속 중 추가 피격을 받으면 `루시드 낙인 3단계`로 강화된다. |
| `LMK-003` | 낙인 지속 중에는 꿈식자가 플레이어를 더 오래 기억하며, 추격 해제가 어려워진다. |
| `LMK-004` | 2단계 강화 시 플레이어의 실시간 위치가 주변 미인지 꿈식자들에게 공유됩니다. 이 상태는 플레이어 시점에서는 완전 비가시적(VFX/UI 연출 없음)이며, 오직 꿈식자들의 내부 감각 필터에만 인식되는 영적 신호로 작동하여 적들을 플레이어 근처로 점진적 수색 유도한 뒤 직접 시야 확보 시 추격하게 만듭니다. |
| `LMK-005` | 루시드 낙인은 이동 속도 감소, 긴 경직, 상호작용 강제 취소를 발생시키지 않는다. |
| `LMK-006` | 낙인 적용 시 화면 외곽을 어둡게 하는 비네팅(Vignette) 효과가 단계별로 차등 인가(1단계: 0.25, 2단계: 0.50, 3단계: 0.75)되어, 손전등 범위 외의 주변 시인성을 크게 제한한다. |
| `LMK-007` | 3단계 진입 시 플레이어의 자아가 심하게 오염되어, 지속적인 불길한 이명 사운드 연출이 런타임에 오버레이 재생된다. |
...
| 낙인 | `lucidMarkStage3Duration` | `float` | 3단계 낙인 지속 시간 |
| 낙인 | `lucidMarkMemoryBonusTime` | `float` | 추격 기억 추가 시간 |
| 낙인 | `lucidMarkLeashBonusDistance` | `float` | 최대 추격 거리 추가값 |
| 낙인 | `lucidMarkVignetteIntensityStage1` | `float` | 1단계 외곽 비네팅 시야 차단 강도 (기본값: 0.25) |
| 낙인 | `lucidMarkVignetteIntensityStage2` | `float` | 2단계 외곽 비네팅 시야 차단 강도 (기본값: 0.50) |
| 낙인 | `lucidMarkVignetteIntensityStage3` | `float` | 3단계 외곽 비네팅 시야 차단 강도 (기본값: 0.75) |
| 누출 | `lucidLeakRange` | `float` | 플레이어의 실시간 위치가 공유되는 의식 누출 반경 (기본값: 8.0) |
...
| `OnLucidLeakPositionShared` | `PlayerLucidMarkReceiver` | `EnemyBrain` (주변 적들) | `Vector3 playerPosition, float range` |
...
| `PlayerLucidMarkReceiver` | 플레이어 낙인 단계 및 지속 시간 관리, 2단계 진입 시 의식 누출 위치 정보 전송 처리 | 신규 생성 |
...
- **의식 누출의 역할 제한**: 루시드 의식 누출은 플레이어 시점에서는 완전 비가시적(VFX/UI 없음)이며 오직 에너미에게만 감지되는 순수 내부 시스템적 연동 사양입니다. 2단계 낙인 발동 시 주변 미인지 적들이 플레이어 위치로 점진적 수색 접근하도록 유도하는 규칙으로 작동합니다.
...
| `QA-005` | 낙인 지속 중 추가 피격 시 2단계로 강화되고, 주변 적에게 실시간 위치를 지속 전송하는 의식 누출 상태가 활성화된다. |
...
| `QA-010` | 루시드 낙인 1단계, 2단계 및 3단계 중첩 시 화면 외곽 비네팅(어둠 왜곡) 강도가 설정 수치(0.25, 0.50, 0.75)에 맞게 실시간 연동되어 스크린 시인성이 감소하고, 3단계 진입 시 이명 사운드가 지속 출력되는지 확인한다. |
