...
| `LMK-004` | 2단계 강화 시 플레이어의 실시간 위치가 주변 미인지 꿈식자들에게 지속적으로 공유되어, 적들이 플레이어 근처로 접근하고 직접 시야 확보 시 즉시 추격을 개시한다. |
...
- **의식 누출의 역할 제한**: 루시드 의식 누출은 시각/청각적 이펙트가 없는 순수 내부 시스템적 연동 사양입니다. 2단계 낙인 발동 시 주변 미인지 적들이 플레이어 위치로 서색 접근하게 만드는 규칙으로 작동합니다.
...
| `QA-006` | 의식 누출 상태는 밖으로 표출되는 연출 없이, 반경 내의 미인지 적들을 플레이어 근처로 유도하여 직접 시야 확보 시 추격하게 만든다. |
