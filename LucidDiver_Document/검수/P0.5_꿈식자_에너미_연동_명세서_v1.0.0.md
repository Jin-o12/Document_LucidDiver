...
flowchart TD
    ...
    AttackSlash2 --> OnHitMark2[낙인 중 추가 피격 시 Stage 2 + Scar]
...
| `LMK-004` | 2단계 강화 시 피격 위치에 `루시드 상흔`이 생성되며, 주변 꿈식자는 이 흔적을 조사한다. |
...
| 상흔 | `lucidScarDuration` | `float` | 상흔 지속 시간 |
| 상흔 | `lucidScarInvestigateRange` | `float` | 주변 적 조사 유도 반경 |
...
|---|---|---|---|
| `OnNoiseEmitted` | `PlayerMovement`, `PlayerWeapon`, `EnemyBrain`, `LucidScarEmitter` | `EnemyNoiseListener` | `Vector3 position, NoiseType type, float range, GameObject source` |
...
| `OnLucidScarSpawned` | `PlayerLucidMarkReceiver` 또는 `EnemyCombat` | `EnemyNoiseListener`, VFX | `Vector3 position, float duration, float range` |
...
| `PlayerLucidMarkReceiver` | 플레이어 낙인 단계, 지속 시간, 상흔 발생 처리 | 신규 생성 |
| `LucidScarEmitter` | 상흔 시각효과 및 조사 단서 송신 | 신규 생성 또는 `PlayerLucidMarkReceiver` 내부 처리 |
...
- **상흔의 역할 제한**: 루시드 상흔은 조사 단서이며, 즉시 광역 경보나 확정 발각 장치가 아니다.
...
| `QA-005` | 낙인 지속 중 추가 피격 시 2단계로 강화되고, 피격 위치에 루시드 상흔이 생성된다. |
| `QA-006` | 루시드 상흔은 주변 꿈식자를 조사 상태로 유도한다. |
...
| 2026-07-01 | v1.0.1 | 먹잇감 차단 이동 규칙 추가, 루시드 낙인 및 상흔 조사 시스템 추가 | AI(Antigravity) |
